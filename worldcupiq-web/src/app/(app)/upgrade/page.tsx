...
function PlanCard({
  ...
  return (
    ...
    >
      ...
      {badge && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2">
          <Badge className="bg-accent text-accent-foreground px-3">{badge}</Badge>
        </div>
      )}
      <CardHeader className="pt-8 pb-4">
        ...
        <div className="mt-3">
          <div className="flex items-baseline gap-1">
            <span className="text-3xl font-extrabold">{price.intl}</span>
            <span className="text-muted-foreground text-sm">/ {period}</span>
          </div>
          <p className="text-xs text-muted-foreground mt-1">₦{price.ng} for Nigerian users</p>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <Button
          className={cn('w-full', isPopular && 'bg-primary', badge && 'bg-accent text-accent-foreground hover:bg-accent/90')}
...
export default function UpgradePage() {
  ...
  const [selectedPlan, setSelectedPlan] = useState<'premium' | 'tournament_pass' | null>(null);
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <div className="mb-10 text-center">
        ...
        <h1 className="text-3xl font-extrabold">Unlock the full WorldCupIQ experience</h1>
        <p className="mt-3 text-muted-foreground">
          Difficulty selection, streak shields, category filters, country leaderboards, and more.
        </p>
        {user?.country && (
          <p className="mt-1 text-xs text-muted-foreground">
            Payments for {user.country === 'NG' ? 'Nigeria' : 'your region'} processed by{' '}
            <strong>{user.country === 'NG' ? 'Paystack' : 'Flutterwave'}</strong>
          </p>
        )}
      </div>
      ...
      <Card className="border-border bg-card overflow-hidden">
        <div className="grid grid-cols-3 text-sm">
          ...
          <div className="p-4 text-center font-medium text-muted-foreground border-l border-border">Free</div>
          <div className="p-4 text-center font-medium text-primary border-l border-border">Premium</div>
        </div>
        <Separator />
        {[
          ...
          <div key={i} className={cn('grid grid-cols-3 text-sm', i % 2 === 0 && 'bg-muted/30')}>
            <div className="col-span-1 p-4">{feature}</div>
            <div className="p-4 text-center border-l border-border text-muted-foreground">
              {free === true ? <Check className="h-4 w-4 text-primary mx-auto" /> : free === false ? '—' : free}
            </div>
            <div className="p-4 text-center border-l border-border">
              {premium === true ? <Check className="h-4 w-4 text-primary mx-auto" /> : premium === false ? '—' : premium}
            </div>
          </div>
        ))}
      </Card>
      ...
      <p className="mt-8 text-center text-xs text-muted-foreground">
        ...
        <a href="mailto:support@worldcupiq.com" className="text-primary hover:underline">
          Contact support
        </a>
      </p>
    </div>
