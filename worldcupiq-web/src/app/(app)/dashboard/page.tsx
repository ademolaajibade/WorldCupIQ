...
export default function DashboardPage() {
  ...
  const hasPlayedToday = user?.stats?.lastChallengeDate === format(new Date(), 'yyyy-MM-dd');
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="mb-8 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        ...
        {user?.subscription?.plan === 'free' && (
          <Button asChild className="self-start bg-accent text-accent-foreground hover:bg-accent/90 sm:self-auto">
            <Link href="/upgrade">
      ...
      {/* Daily Challenge CTA */}
      <Card className={`mb-8 overflow-hidden border-border ${hasPlayedToday ? 'opacity-75' : 'bg-gradient-to-r from-primary/20 to-primary/5'}`}>
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {statsLoading ? (
          [...Array(isPremium ? 5 : 4)].map((_, i) => <Skeleton key={i} className="h-24 rounded-xl" />)
        ) : (
          <>
            <StatCard
              icon={Trophy}
                ...
                <CardContent className="flex items-center gap-4 p-5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {shields === 0 ? 'None left' : shields === 1 ? '1 miss protected' : `${shields} misses protected`}
                    </p>
                  </div>
                </CardContent>
              </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="border-border bg-card lg:col-span-2">
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <div className="mb-1.5 flex justify-between text-sm">
                <span className="text-muted-foreground">Daily challenges completed</span>
                <span className="font-medium">{user?.stats?.dailyChallengesCompleted ?? 0}</span>
              </div>
              <Progress
                value={Math.min((user?.stats?.dailyChallengesCompleted ?? 0) * 2, 100)}
                className="h-2"
              />
            </div>
          </CardContent>
